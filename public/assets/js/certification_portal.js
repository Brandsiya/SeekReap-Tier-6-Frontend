...
const typeAccept = { 
  ...
  epub: '.epub', 
  pdf: '.pdf', 
  code: '.js,.ts,.py,.html,.css,.json,.java,.c,.cpp,.rb,.php,.go,.rs,.txt' 
};
...
function updateStepUI() {
  ...
  if (mode === 'solo') {
    ...
    s4Title.textContent = 'Upload Work';
    if (s4Header) s4Header.textContent = 'Upload Work';
    s4Sub.textContent = 'Upload your work for blockchain certification';
  } else {
    s3Icon.innerHTML = '<i class="fas fa-users"></i>';
    s4Title.textContent = 'Upload & Invite';
    if (s4Header) s4Header.textContent = 'Upload & Invite';
...
function processFile(file, area, listId, previewId, previewLabelId, previewTypeId, previewBodyId, progressId, barId, textId) {
  ...
  simulateUpload(progressId, barId, textId, () => {
    ...
    if (list) {
      list.innerHTML = `<div class="file-item"><span>${icon}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${file.name}</span><span class="file-size">${formatBytes(file.size)}</span></div>`;
    }
...
function renderPreview(type, file, previewId, previewLabelId, previewTypeId, previewBodyId) {
  ...
  switch (type) {
    case 'audio': renderAudioPreview(body, url); break;
    ...
    default:
      body.innerHTML = `<span style="color:var(--white-dim);font-size:0.85rem;"><i class="fas fa-file" style="margin-right:8px;"></i> ${file.name} uploaded successfully</span>`;
  }
}

function renderAudioPreview(body, url) {
...
function renderImagePreview(body, url) {
  ...
  body.innerHTML = `<div class="image-preview-wrap">
    ...
    <div class="image-zoom-controls">
      <button class="zoom-btn" onclick="window.changeZoom(-0.2)"><i class="fas fa-search-minus"></i></button>
      <button class="zoom-btn" onclick="window.changeZoom(0)"><i class="fas fa-sync-alt"></i></button>
      <button class="zoom-btn" onclick="window.changeZoom(0.2)"><i class="fas fa-search-plus"></i></button>
    </div>
  </div>`;
}
...
function renderPdfPreview(body, file) {
  ...
  body.innerHTML = `<div class="pdf-preview-wrap">
    <div class="pdf-nav">
      <button id="pdfPrev" onclick="window.pdfPage(-1)"><i class="fas fa-chevron-left"></i></button>
      <span class="pdf-page-info" id="pdfInfo">Loading…</span>
      <button id="pdfNext" onclick="window.pdfPage(1)"><i class="fas fa-chevron-right"></i></button>
    </div>
  ...
  reader.onload = e => {
    pdfjsLib.getDocument(new Uint8Array(e.target.result)).promise.then(doc => {
      ...
      body.innerHTML = '<span style="color:var(--danger);font-size:0.85rem;"><i class="fas fa-exclamation-triangle" style="margin-right:6px;"></i> Could not render PDF</span>';
    });
  };
...
function renderEpubPreview(body, file) {
  ...
  <div style="display:flex;gap:8px;margin-top:10px;justify-content:center;">
    <button class="zoom-btn" onclick="window.epubNav(-1)"><i class="fas fa-chevron-left"></i> Prev</button>
    <button class="zoom-btn" onclick="window.epubNav(1)">Next <i class="fas fa-chevron-right"></i></button>
  </div>`;
...
window.epubNav = function(dir) {
  if (window._epubRendition) {
    dir > 0 ? window._epubRendition.next() : window._epubRendition.prev();
  }
};
...
function renderCollaborators() {
  ...
  document.querySelectorAll('.split-slider-input').forEach(s => s.addEventListener('input', e => {
    ...
    if (c) {
      ...
      const sp = document.querySelector('.split-pct-' + id);
      if (sp) sp.textContent = c.split + '%';
      updateSplitSummary();
    }
  }));
...
function updateSplitSummary() {
  ...
  collaborators.forEach((c, i) => {
    html += `<div class="split-piece"><div class="split-circle" style="background:${colors[(i + 1) % colors.length]};"></div><div class="split-label">${c.email.split('@')[0]} · ${c.split}%</div></div>`;
  });
...
document.getElementById('backToStep3Btn').addEventListener('click', () => showStep(3));

document.getElementById('nextToOwnershipBtn').addEventListener('click', () => {
  if (!document.getElementById('workTitle').value.trim()) {
...
document.getElementById('addCollaboratorBtn').addEventListener('click', () => {
  ...
  if (!email || !email.includes('@')) {
    if (email !== null) alert('Please enter a valid email.');
    return;
  }
  ...
  if (isNaN(split) || split < 1 || split > maxSplit) {
    alert('Invalid split percentage.');
    return;
  }
...
document.getElementById('finalizeBtn').addEventListener('click', () => {
  ...
  if (mode === 'solo') {
    ...
    if (collaborators.length === 0) {
      alert('Please add at least one co-owner.');
      return;
    }
    const inviteList = document.getElementById('inviteList');
    if (inviteList) {
      inviteList.innerHTML = collaborators.map(c => `<div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:0.85rem;color:var(--white-dim);"><i class="fas fa-envelope" style="margin-right:8px;color:var(--gold);"></i>${c.email} — ${c.split}% share</div>`).join('');
    }
